      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNumber" @change="skuNumber = skuNumber>=1? parseInt(skuNumber):1">
  ...
  export default {
    name: 'Detail',
      ...
      methods:{
         ...
         async addShopCart(){
                let {skuId,skuNumber} = this
                //dispatch 分发触发的意思 其实就是调用actions函数
                //this.$store.dispatch('addOrUpdateCart',{skuId,skuNumber})就是函数调用表达式，结果就是
                //actions当中异步函数的返回值,异步函数的返回值一定是promise

            try {
                await this.$store.dispatch('addOrUpdateCart',{skuId,skuNumber})
                alert('添加购物车成功')
                sessionStorage.setItem('SKUINFO_KEY',JSON.stringify(this.skuInfo))
                this.$router.push('/addcartsuccess?skuNumber='+this.skuNumber)
            }catch (error){
                alert(error.message)
            }
